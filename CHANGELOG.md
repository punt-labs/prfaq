...
- `make check` gained a `test-prose` target (131 tests covering the scanner, the LaTeX masking, the hook's scope guard, and the banned patterns) and a `lint-prose` target, informational, that runs the scanner against `prfaq.tex` and the reference guides for repo-local dogfooding.
