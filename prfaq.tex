...
\prfaqversion{3}{0}  % v3.0 — restructure TAM (reachable-first), reorganize Solution by workflow stage, real customer quote, soften vote framing, cite semianalysis
...
\begin{faqpair}{What is the total addressable market?}\label{faq:tam}
  \textbf{Reachable users today.} The plugin runs on Claude Code. Claude Code's trajectory --- surpassing \$2.5B in annualized revenue~\cite{anthropic2026revenue} and doubling since January --- indicates a large and growing base. Exact user counts are not public; a rough revenue/ARPU inference suggests 2--5M Claude Code users as of Q1 2026 (assuming a blended ARPU of \$40--100/month across Pro, Max 5$\times$, Max 20$\times$, and API users), though this estimate carries significant uncertainty. Of those, the product-decision-making subset (founders, technical leads, PMs) is an assumed 10--25\% --- an estimate we have not validated and plan to test through the validation trial (\faqref{faq:validation}). That yields 200K--1.25M reachable users today.

  This is not a TAM-based revenue argument (the plugin is free); it is a reach argument. The question is how many builders the plugin can help, not how many it can monetize. The trajectory of both Claude Code adoption and the broader vibe-coding wave suggests this range grows significantly over 12--18 months.
...
\begin{faqpair}{What are the major technical risks?}
  The full feature set through v1.1.0 is shipped, including multi-agent review meetings (v0.6.0--v0.8.0), go/no-go investment verdicts with evidence decomposition and investment proportionality calibration (v1.0.0--v1.1.0), and customer-facing press release generation (v1.0.0). The plugin is a set of markdown files plus a LaTeX template and a shell script. No backend, no API, no database. The LaTeX dependency requires a TeX distribution installed, a mild friction point; the installer mitigates with platform-specific setup instructions.
