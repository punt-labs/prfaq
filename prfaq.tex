...
\begin{itemize}[nosep]
  ...
  \item Reference guides --- encoded best practices for PR structure, FAQ structure, four risks, common mistakes, unit economics, feasibility, and usability
  \item Peer review agent --- automated critique against Working Backwards principles and cognitive bias checklist
  \item Research sub-agent --- scans local \texttt{./research/} directory, indexed documents, and the web for evidence
  \item Biblatex citation system --- academic-style citations for claims, linking evidence to assertions
  \item \texttt{/prfaq:feedback} --- directed iteration from pointed user feedback; traces cascading effects via section dependency graph and surgically redrafts affected sections
  \item \texttt{/prfaq:import} --- ingest an existing PR/FAQ draft (plain text, Markdown, or raw \texttt{.tex}), parse and structure it against the framework, research evidence for unsupported claims, format into the LaTeX template with proper environments and citations, compile to PDF, and present a gap analysis
\end{itemize}

\subsection*{Should Do (V2)}

\begin{itemize}[nosep]
  \item \texttt{/prfaq:meeting} --- simulate an Amazon-style PR/FAQ review meeting. The document is reviewed page by page, FAQ by FAQ, with agentic personas (target customer, principal engineer, unit economics reviewer, UX bar raiser, skeptical executive). Agent debate is visible to the user. Main Claude facilitates and synthesizes debate into pointed decision questions. The user is the PM and final decision-maker. Decisions feed into \texttt{/prfaq:feedback} for automated iteration. This is the most ambitious planned feature and carries meaningful execution risk in multi-agent orchestration and persona fidelity.
