...
% ── Headline block ───────────────────────────────────────────────────────────
\begin{center}
  {\LARGE\bfseries\color{SectionBlue} punt-labs Releases prfaq, a Free Claude Code Plugin\\That Brings Amazon's PR/FAQ Process to Builders} \\[0.8em]
  {\large\color{AccentGray} Open-source plugin guides builders and founders through product discovery\\inside the terminal, producing a professionally typeset decision document in under an hour}
\end{center}
...
\begin{faqpair}{What is prfaq and who is it for?}
  \texttt{prfaq} is a free Claude Code plugin that guides builders and founders through the Amazon Working Backwards PR/FAQ process. It is for anyone who uses AI tools to create products and wants to validate an idea before committing weeks or months of development time. Whether you are an engineer, a technical founder, or a non-technical builder who uses Claude Code as a sparring partner --- if you make product decisions, this tool is for you.
\end{faqpair}
...
\begin{faqpair}{What is the total addressable market?}\label{faq:tam}
  The primary market is builders and founders who use AI coding tools to create products. This audience is broader than professional engineers. As of early 2026, 84\% of professional developers use or plan to use AI tools in their workflows, with 51\% relying on them daily~\cite{stackoverflow2025survey}. But Claude Code's trajectory --- surpassing \$2.5B in annualized revenue~\cite{anthropic2026revenue} and doubling since January --- suggests the tool is crossing the chasm beyond traditional developers to product-minded founders, consultants, and operators who use AI as a building and thinking partner. The subset of Claude Code users who make product decisions --- founders, indie hackers, startup engineers, technical co-founders, and increasingly non-technical builders --- is large and growing, though exact counts are not publicly available. Additionally, the \texttt{/prfaq:import} flow serves users who adopt Claude Code as a sparring partner for strategic thinking, not just code generation. This is not a TAM-based revenue argument (the plugin is free); it is a reach argument. A useful, free plugin in a fast-growing ecosystem reaches users through organic discovery, and the audience grows as Claude Code's user base diversifies.
\end{faqpair}

\begin{faqpair}{What evidence do we have that customers want this?}\label{faq:customer-evidence}
  Three categories of indirect evidence, but no primary customer data yet. First, the ``vibe coding'' phenomenon: thousands of products are being built at speed without product discipline, and rebuilds are already visible across startup communities and forums. Second, the explosion of ``how to think about product'' content targeting builders --- books, courses, and newsletters suggest unmet demand for product skills among people who build with AI tools but lack formal product training. Third, the Working Backwards process itself has two decades of validation at Amazon~\cite{bryar2021workingbackwards}, where it is mandatory for new product proposals. The gap is not whether the framework works --- it is accessibility. Builders do not attend PM workshops. They install plugins. \textit{Note: we have not yet validated that builders want product frameworks delivered inside Claude Code. Five to ten customer discovery interviews --- spanning both engineers and non-technical builders --- would significantly strengthen this evidence base.}
\end{faqpair}
...
\begin{faqpair}{Why now? What has changed?}
  Three shifts converged. First, AI coding tools reached mainstream adoption in 2025--2026, creating a large population of builders who can create products independently but lack product discipline --- and this population now extends beyond professional engineers to founders, operators, and domain experts who use AI as a building partner. Second, Claude Code's plugin system matured, making it possible to deliver structured workflows inside the builder's existing tool. Third, the consequences of ``vibe coding without product thinking'' are becoming visible --- failed launches, rebuilds, wasted months --- creating demand for lightweight product frameworks that do not require hiring a PM or leaving the terminal.
\end{faqpair}
...
\midrule
Value      & \textcolor{RiskAmber}{Medium} & Market-level evidence strong (proven framework, growing and diversifying audience). Customer-level evidence absent --- no interviews confirming demand for this delivery mechanism from either engineers or non-technical builders. Plugin is free, so adoption barrier is low. \\
