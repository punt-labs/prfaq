...
\texttt{prfaq} is a Claude Code plugin (v1.5.2) that implements Amazon's Working
...
The codebase has six component categories:
...
\begin{description}[style=nextline, leftmargin=2em]
  ...
  \item[\texttt{commands/}]
    Thirteen slash commands for post-generation operations (review, meeting,
  ...
  \item[\texttt{scripts/}]
    Four shell scripts handling PDF compilation, Word export, LaTeX
    preprocessing for pandoc, and reference document generation.
\end{description}
...
The quality gate is compilation of two LaTeX files:

\begin{enumerate}[nosep]
  \item \texttt{prfaq.tex} --- the plugin's own PR/FAQ (dogfood document)
  \item \texttt{assets/prfaq-template.tex} --- the template used to generate new documents
\end{enumerate}

Both must compile to valid PDFs via \texttt{compile\_prfaq.sh} (which runs
\texttt{pdflatex} $\to$ \texttt{biber} $\to$ \texttt{pdflatex} $\times$ 2)
before every commit. The \texttt{.docx} export pipeline preprocesses custom
LaTeX environments into standard LaTeX, then hands off to pandoc.
