...
- **`/prfaq:meeting`** — Interactive review meeting. Alex (Skeptical Executive) opens with a framing read before the agenda and closes with a verdict plus a next-step/reconvene proposal after every hot spot has a decision. In between, four personas (principal engineer, target customer, skeptical executive, builder-visionary) debate the document's hot spots. The user makes explicit tradeoff decisions at each disagreement: KEEP, REVISE, RESEARCH, or DEFER. Good for forcing decisions. Output is a decisions log — including Alex's opening/closing assessment — with specific revision directives.
- **`/prfaq:meeting-hive`** — Autonomous consensus meeting. Same opening/closing assessment, same four personas, but they debate and reach consensus without user moderation. Uses one-way/two-way door framework to weight caution vs. action. Persistent splits over irreversible decisions are recorded during the autonomous run and presented to the user in one batch afterward (REVISE/KEEP/DEFER) — before Alex's closing assessment, so the closing read always reflects the fully-resolved picture. Requires Agent Teams (`/prfaq:permissions` sets `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in the project's `.claude/settings.json`).
